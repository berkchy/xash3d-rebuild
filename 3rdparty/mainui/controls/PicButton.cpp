...
void CMenuPicButton::Draw( )
{
	...
	if( hPic && !uiStatic.renderPicbuttonText && !m_bForceText )
	{
		int r, g, b;
	...
	else if( !uiStatic.lowmemory )
	{
		...
		CColor light_blur_color, heavy_blur_color;
		Point pos = m_scPos;

		if( iFlags & QMF_GRAYED )
		{
			heavy_blur_color = uiColorDkGrey;
			light_blur_color = InterpColor( uiColorBlack, uiColorDkGrey, 0.333f ); // because additive, tone down all channels
		}
		else if( this == m_pParent->ItemAtCursor() )
		{
			heavy_blur_color = colorFocus;
			light_blur_color = colorFocus;
		}
		else
		{
			heavy_blur_color = colorBase;
