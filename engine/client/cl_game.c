...
// engine callbacks
static unsigned int cl_sharedSeed = 0;

unsigned int cl_sharedSeedTable[ 256 ] =
{
...
static unsigned int CL_SharedRandomNext( void )
{
	cl_sharedSeed *= 69069;
	cl_sharedSeed += cl_sharedSeedTable[ cl_sharedSeed & 0xff ];

	return ( ++cl_sharedSeed & 0x0fffffff );
}

static void CL_SharedSrand( unsigned int seed )
{
	cl_sharedSeed = cl_sharedSeedTable[ seed & 0xff ];
}
...
static float GAME_EXPORT CL_SharedRandomFloat( unsigned int seed, float low, float high )
{
	...
	CL_SharedSrand( (int)seed + *(int *)&low + *(int *)&high );
